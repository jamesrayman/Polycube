...
DancingChain::DancingChain (const std::vector<std::vector<char>>& candidates) {
    int nCells = candidates[0].size(), nCands = candidates.size();
    cellsActive = nCells;
    ...
    for (int col = 0; col <= nCells; col++) {
        ...
        for (int row = 0; row <= nCands; row++) {
            ...
            else linkActive = candidates[row-1][col-1];
...
int DancingChain::mostConstrained () const {
    ...
    for (int index = right(0, 0); index != 0; index = right(index, 0)) {
        if (at(index, 0).size < at(bestIndex, 0).size)
            bestIndex = index;
    }
